...
echo "[Zeek Entrypoint] Verificação final dos serviços:"
echo "[Zeek Entrypoint] - nginx status: $(ps -p $NGINX_PID >/dev/null && echo 'RUNNING' || echo 'STOPPED')"
echo "[Zeek Entrypoint] - PHP-FPM status: $(ps aux | grep php-fpm | grep -v grep >/dev/null && echo 'RUNNING' || echo 'STOPPED')"
echo "[Zeek Entrypoint] - Porta 8090: $(netstat -tulpn | grep -q ":8090" && echo 'LISTENING' || echo 'NOT LISTENING')"
